...
def main() -> int:
    ...
    # Bundle / extension display names only. No PresentationData, UIKit trait,
    # LiquidGlass or theme-selection code is touched in this diagnostic build.
    ...
    for path in localized_plists:
        ...
        if display_pattern.search(text):
            text = display_pattern.sub('"CFBundleDisplayName" = "AyuGram";', text, count=1)
        else:
            text = '"CFBundleDisplayName" = "AyuGram";\n' + text
        if name_pattern.search(text):
            text = name_pattern.sub('"CFBundleName" = "AyuGram";', text, count=1)
        else:
            text = '"CFBundleName" = "AyuGram";\n' + text
        path.write_text(text, encoding="utf-8")
    ...
    if "AYU_NATIVE_APPEARANCE_SYNC_v0_3" in (root / "submodules/TelegramUI/Sources/SharedAccountContext.swift").read_text(encoding="utf-8"):
        raise RuntimeError("diagnostic build unexpectedly contains native appearance/theme patch")
    if "AYU_IOS27_LIQUID_LENS_FALLBACK_v0_3" in (root / "submodules/TelegramUI/Components/LiquidLens/Sources/LiquidLensView.swift").read_text(encoding="utf-8"):
        raise RuntimeError("diagnostic build unexpectedly contains LiquidLens patch")

    print("[ayu-branding-only] AyuGram names installed; Telegram theme/LiquidGlass code untouched")
    return 0
